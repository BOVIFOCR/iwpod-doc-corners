...
def l1(true, pred, szs):
    b,h,w,ch = szs
    #res = tf.reshape((true-pred)**2, (b,h*w*ch)) # l2 (new)
    res = tf.reshape(true-pred, (b,h*w*ch)) # l1 (original)
    res = tf.abs(res)
...
def mse_loss(Ytrue, Ypred):
    ...
    wclas = 0.1
    wloc = 0.9
    return wloc*loc_loss(Ytrue, Ypred) + wclas*mse_calc(Ytrue, Ypred)
