...
def augment_sample(I, shapelist, dim, maxangle = 2 * np.array([65.,65.,55.]), maxsum = 140):
    ...
    #     I, pts = flip_image_and_pts(I, ptzs)
    #     print(pts)
    #
    ...
    #    
    #if np.random.rand() < 0.15:
    #    I = cv2.resize(I, None, fx=1/2, fy=1/2)
    ...
    if (pts[0][1] - pts[0][0] > .002): ## if not fake plate
        ...
        #
        #whratio = 1.5364 # averaged from anonymized RG dataset
        #if vtype == 'bike':
        #    whratio = random.uniform(1.25, 2.5)
        #else:
        #    whratio = random.uniform(2.5, 3.5)
        whratio = random.uniform(1.4, 1.6)

        #
        #  Width of LP in training image 
        #
        dim0 = 130
        #dim0 = int(0.38 * 208) #130 # augments data w.r.t. a fixed resolution
        #dim1 = int(0.98 * 208)        

        #
        #  Defines range of LP widths w.r.t to baseline resolution dim0 = 208
        #
        #wsiz = random.uniform(dim0*1.0, dim1) #dim0*1.8)
        wsiz = random.uniform(dim0*1.0, dim0*1.8)
        ...
        # #  Width of LP in training image 
        # #
        # dim0 = 64 # augments data w.r.t. a fixed resolution
        
        # #
        # #  Defines range of LP widths w.r.t to baseline resolution dim0 = 208
        # #
        # wsiz = random.uniform(dim0*1.2, dim0*1.6)
