...
if __name__ == '__main__':
	...
	ocr_input_size = [100, 150] # desired LP size (width x height)
	
	#
	#  Loads network and weights
	#
	#iwpod_net = load_model('results/plate_w1/iwpodnet_plates')
	iwpod_net = load_model('results2/wild/fold_10_1/iwpodnet_wild_1_10')
	...
	if (vtype in ['car', 'bus', 'truck']):
		...
		#
		ASPECTRATIO = 1 
		WPODResolution = 208 # larger if full image is used directly
		lp_output_resolution =  tuple(ocr_input_size[::-1])
	else:
		#
	...
	#
	cv2.imwrite("image_and_lps.png", Ivehicle)
